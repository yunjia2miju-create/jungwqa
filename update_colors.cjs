...
function replaceColors(content) {
    ...
    // The user wants 'Deep Kings Navy, #0B2545' !important
    newContent = newContent.replace(/style=\{\{ background: '#0B2545', backgroundColor: '#0B2545', borderColor: '#0B2545' \}\}/g, "style={{ background: '#0B2545 !important', backgroundColor: '#0B2545 !important', borderColor: '#0B2545 !important' }}");

    // [2] Search box icon bg
    newContent = newContent.replace(/style=\{\{ background: '#0B2545 !important', backgroundColor: '#0B2545 !important' \} as React.CSSProperties\}/g, "style={{ background: '#0B2545 !important', backgroundColor: '#0B2545 !important', borderColor: '#0B2545 !important' } as React.CSSProperties}");

    // DetailTab Fact sheet icon
    newContent = newContent.replace(/<i className="fa-solid fa-building-circle-check text-sm lg:text-base" style=\{\{ color: '#0B2545' \}\}/g, "<i className=\"fa-solid fa-building-circle-check text-sm lg:text-base\" style={{ color: '#0B2545 !important' }}");

    // Fact sheet manage fee
    newContent = newContent.replace(/<span className="font-extrabold lg:text-base" style=\{\{ color: '#0B2545' \}\}/g, "<span className=\"font-extrabold lg:text-base\" style={{ color: '#0B2545 !important' }}");

    // App.tsx header logo text
    newContent = newContent.replace(/style=\{\{ color: '#0B2545' \}\}/g, "style={{ color: '#0B2545 !important' }}");
    newContent = newContent.replace(/style=\{\{ backgroundColor: '#0B2545' \}\}/g, "style={{ backgroundColor: '#0B2545 !important' }}");

    // [2.1] 활성화된 '전체' 분류 버튼 잔상 
    // From: active bg-[#0B2545] text-white border-[#0B2545] scale-[1.03] shadow-[0_6px_20px_rgba(11,37,69,0.35)] ring-2 ring-[#0B2545]/20
    // To: Ensure #0B2545 !important is used.
    newContent = newContent.replace(/active bg-\[#0B2545\] text-white border-\[#0B2545\] scale-\[1.03\] shadow-\[0_6px_20px_rgba\(11,37,69,0.35\)\] ring-2 ring-\[#0B2545\]\/20/g, 
    ...
    newContent = newContent.replace(/text-emerald-400 font-black text-xl sm:text-2xl/g, "text-[#0B2545] font-black text-xl sm:text-2xl");
    newContent = newContent.replace(/text-emerald-300 text-\[10px\]/g, "text-[#0B2545] text-[10px]");
    newContent = newContent.replace(/text-emerald-400 animate-pulse/g, "text-[#0B2545] animate-pulse");
    ...
    */
    ...
    newContent = newContent.replace(/<Naver360Icon size=\{144\} className="h-\[57px\] sm:h-\[90px\] w-auto shrink-0 select-none animate-vr-glow filter drop-shadow-\[0_4px_12px_rgba\(11,37,69,0.35\)\]" \/>/g, 
        `<div className="relative flex flex-col items-center justify-center w-[57px] h-[57px] sm:w-[90px] sm:h-[90px] rounded-full bg-[#0B2545] border-2 border-white/20 shadow-[0_4px_12px_rgba(11,37,69,0.35)] shrink-0 select-none animate-vr-glow">
    ...
    newContent = newContent.replace(/<Naver360Icon size=\{12\} className="text-\[#0B2545\]" \/>/g,
        `<Home size={12} className="text-[#0B2545]" strokeWidth={1.8} />`);
    ...
    newContent = newContent.replace(/bg-\[#00925c\]/g, "bg-[#0B2545]");
