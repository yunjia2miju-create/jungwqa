...
content = content.replace(/matchingRecs\.slice\(0, 18\);/g, 'matchingRecs.slice(0, 6);');

// Next find the grid and replace the card markup
const startMarker = '<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">';
const endMarker = '                    </div>\n                </div>\n            )}';
...
const newCode = `<div className="flex flex-wrap justify-center gap-6 sm:gap-8">
